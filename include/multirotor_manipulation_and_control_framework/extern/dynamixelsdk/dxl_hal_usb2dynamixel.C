...
int dynamixelsdk::dxl_hal_open(int deviceIndex, float baudrate)
{
...
void dynamixelsdk::dxl_hal_close()
{
	if(gSocket_fd != -1)
		close(gSocket_fd);
	gSocket_fd = -1;
...
int dynamixelsdk::dxl_hal_set_baud( float baudrate )
{
	struct serial_struct serinfo;
...
void dynamixelsdk::dxl_hal_clear(void)
{
	tcflush(gSocket_fd, TCIFLUSH);
}

int dynamixelsdk::dxl_hal_tx( unsigned char *pPacket, int numPacket )
{
	return write(gSocket_fd, pPacket, numPacket);
}

int dynamixelsdk::dxl_hal_rx( unsigned char *pPacket, int numPacket )
{
...
void dynamixelsdk::dxl_hal_set_timeout( int NumRcvByte )
{
	glStartTime = myclock();
	gfRcvWaitTime = (float)(gfByteTransTime*(float)NumRcvByte + 5.0f);
}

int dynamixelsdk::dxl_hal_timeout(void)
{
	long time;
