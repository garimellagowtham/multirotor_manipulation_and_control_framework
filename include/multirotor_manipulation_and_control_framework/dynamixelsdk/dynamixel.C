...
int dynamixelsdk::dxl_initialize(int deviceIndex, int baudnum )
{
	float baudrate;	
...
void dynamixelsdk::dxl_terminate(void)
{
	dxl_hal_close();
}

void dynamixelsdk::dxl_tx_packet(void)
{
	unsigned char i;
...
void dynamixelsdk::dxl_rx_packet(void)
{
...
void dynamixelsdk::dxl_txrx_packet(void)
{
	dxl_tx_packet();
...
int dynamixelsdk::dxl_get_result(void)
{
	return gbCommStatus;
}

void dynamixelsdk::dxl_set_txpacket_id( int id )
{
	gbInstructionPacket[ID] = (unsigned char)id;
}

void dynamixelsdk::dxl_set_txpacket_instruction( int instruction )
{
	gbInstructionPacket[INSTRUCTION] = (unsigned char)instruction;
}

void dynamixelsdk::dxl_set_txpacket_parameter( int index, int value )
{
	gbInstructionPacket[PARAMETER+index] = (unsigned char)value;
}

void dynamixelsdk::dxl_set_txpacket_length( int length )
{
	gbInstructionPacket[LENGTH] = (unsigned char)length;
}

int dynamixelsdk::dxl_get_rxpacket_error( int errbit )
{
...
int dynamixelsdk::dxl_get_rxpacket_length(void)
{
	return (int)gbStatusPacket[LENGTH];
}

int dynamixelsdk::dxl_get_rxpacket_parameter( int index )
{
	return (int)gbStatusPacket[PARAMETER+index];
}

int dynamixelsdk::dxl_makeword( int lowbyte, int highbyte )
{
	unsigned short word;
...
int dynamixelsdk::dxl_get_lowbyte( int word )
{
	unsigned short temp;
...
int dynamixelsdk::dxl_get_highbyte( int word )
{
	unsigned short temp;
...
void dynamixelsdk::dxl_ping( int id )
{
	while(giBusUsing);
...
int dynamixelsdk::dxl_read_byte( int id, int address )
{
	while(giBusUsing);
...
void dynamixelsdk::dxl_write_byte( int id, int address, int value )
{
	while(giBusUsing);
...
int dynamixelsdk::dxl_read_word( int id, int address )
{
	while(giBusUsing);
...
void dynamixelsdk::dxl_write_word( int id, int address, int value )
{
	while(giBusUsing);
